...
comparisonSections.forEach((section) => {
  ...
  shuffleBtn.addEventListener("click", () => {
    alert("Tilt your Phone");
    section.classList.toggle("positions");
  });
  section.querySelector(".before-image").height =
    section.querySelector(".after-image").height;
  section.querySelector(".image-container").style.height =
    section.querySelector(".after-image").clientHeight + "px";
  ...
  const HidableElements = [
    section.querySelector(".controls-container"),
    section.querySelector(".before-image "),
  ];
  SliderToggle.addEventListener("click", () => {
    HidableElements.forEach(
      (element) => element.classList.toggle("hidden"),
      section.style.setProperty("--position", `50%`)
    );
  });
  section.querySelector(".beforeShow").onclick = () => {
    section.style.setProperty("--position", `100%`);
  };
  section.querySelector(".afterShow").onclick = () => {
    section.style.setProperty("--position", `0%`);
  };
